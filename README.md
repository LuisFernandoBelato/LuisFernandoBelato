# I'm Luis Fernando Oliveira Belato | **Full-Stack Developer**

<!-- ## <a href="https://luisfernandobelato.github.io/Curriculo-Vitae/" title="Curriculum Vitae" target="_blank" rel="noopener noreferrer">Curriculum Vitae</a> -->

## How to Reach Me

<p align="center">
  <a href="https://luisfernandobelato.github.io/Curriculo-Vitae/">
    <img src="https://img.shields.io/badge/Resume-Currículo-7C3AED?style=for-the-badge&logo=safari&logoColor=white"/>
  </a>
  <a href="https://www.linkedin.com/in/luisfernandobelato">
    <img src="https://img.shields.io/badge/LinkedIn-luisfernandobelato-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:luisferbelato@gmail.com">
    <img src="https://img.shields.io/badge/Email-luisferbelato@gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

## Education
### Bachelor's degree in Information Systems at **FIPP - Unoeste (Universidade do Oeste Paulista)**.

## Current Work
Currently working as a **Software Developer (Full-Stack)** on **iBati - Tecnologia e Gestão de Ocorrências**, developing SaaS solutions for the Insurance market (Insurtech).
Responsible for implementing payment integrations (PagBank - PagSeguro API) and internal educational platforms (iBati Academy).

## **Best Undergraduate Student**
### Honored by the **Brazilian Computer Society (SBC)** as the **OUTSTANDING STUDENT** of the 43rd Bachelor of Information Systems Class - SP (FIPP/UNOESTE) 2022 - 2025

**Merit Certificates (Among the Top 5 Averages):**

* 8th Term - 3rd Place
* 7th Term - 1st Place
* 6th Term - 1st Place
* 5th Term - 1st Place
* 4th Term - 2nd Place
* 3rd Term - 1st Place
* 2nd Term - 2nd Place
* 1st Term - 5th Place

## Professional Background
- **Development** - Java, Spring Boot, Python (FastAPI), C, C#, PHP, JavaScript, TypeScript, Node.js, React, Next.js, SolidJS, Vue.js, PL/SQL, MySQL, MongoDB, Docker, Oracle Cloud, Google Cloud Platform (GCP)
- **Tools** - Git, Swagger, Android Studio, MySQL Workbench, Google Colab

## Currently Learning
- Exploratory Data Analysis & Data Science **(Essential Math for Data Science - Thomas Nield)**.

## Technical Skills
| Category        | Technologies                                                               |
|-----------------|----------------------------------------------------------------------------|  
| **Backend**     | Node.js, TypeScript, Spring Boot, ASP.NET, Python (FastAPI), PHP, Docker   |
| **Frontend**    | React, Next.js, SolidJS, Vue.js, jQuery                                    |
| **Cloud/Infra** | Oracle Cloud, Google Cloud Platform (GCP), Azure Cloud (AZ-900)            |

---

<h3 align="left">Languages and Tools:</h3> 
<br>
<div align="center" >
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=git,java,spring,python,fastapi,c,cs,dotnet,javascript,typescript,nodejs,express,nest,react,nextjs,solidjs,vue,php,html,css,mysql,postgres,mongodb,docker,gcp,azure,linux,postman" />
  </a>
  <br />
</div>
